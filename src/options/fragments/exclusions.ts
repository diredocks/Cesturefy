...
function main() {
  const exclusionsContainer = document.getElementById('exclusionsContainer') as HTMLUListElement;
  exclusionsContainer.dataset.noEntriesHint = chrome.i18n.getMessage('exclusionsHintNoEntries');

  const exclusionsForm = document.getElementById('exclusionsForm') as HTMLFormElement;
  exclusionsForm.addEventListener('submit', onFormSubmit);
  const urlPatternInput = exclusionsForm.elements.namedItem('urlPattern') as HTMLInputElement;
  urlPatternInput.placeholder = chrome.i18n.getMessage('exclusionsPlaceholderURL');
...
function onFormSubmit(this: HTMLFormElement, event: Event) {
  ...
  const urlPatternInput = this.elements.namedItem('urlPattern') as HTMLInputElement;
...
function addExclusionsEntry(exclusionsEntry: HTMLLIElement) {
  const exclusionsContainer = document.getElementById('exclusionsContainer') as HTMLElement;
  ...
  exclusionsEntry.addEventListener(
    'animationend',
    (event) => {
      (event.currentTarget as HTMLElement).classList.remove('excl-entry-animate-add');
    },
    { once: true }
  );
  ...
  for (const node of exclusionsContainer.children) {
    ...
    el.addEventListener(
      'transitionend',
      (event) => (event.currentTarget as HTMLElement).style.removeProperty('transition'),
      { once: true }
    );
