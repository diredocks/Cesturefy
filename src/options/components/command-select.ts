...
export class CommandSelect extends HTMLElement {
  ...
  constructor() {
    ...
    this.shadow.getElementById('mainAccessButton')!
      .addEventListener('click', this._handleMainButtonClick.bind(this));
    this.shadow.getElementById('secondaryAccessButton')!
  ...
  private _updateButtons(value: any) {
    ...
    if (value) {
      this._command = Command.fromJSON({ name: value.name, settings: value.settings });
    } else {
      this._command = null;
    }
  ...
  private _buildCommandBar(): DocumentFragment {
    ...
    frag.getElementById('overlay')!.addEventListener('click', () => this._closeCommandBar(), { once: true });
    frag.getElementById('commandBarCancelButton')!.addEventListener('click', () => this._closeCommandBar(), { once: true });
  ...
  private async _buildCommandsPanel(): Promise<HTMLElement> {
    ...
    heading.title = heading.textContent = chrome.i18n.getMessage('commandBarTitle');

    const searchBtn = frag.getElementById('commandsSearchButton')!;
    const searchInput = frag.getElementById('commandsSearchInput') as HTMLInputElement;
    const panel = frag.getElementById('commandsPanel')!;

    searchBtn.addEventListener('click', () => this._toggleSearch(searchInput, panel));
    searchInput.placeholder = chrome.i18n.getMessage('commandBarSearch');
    searchInput.addEventListener('input', () => this._handleSearch(searchInput.value, panel));
    ...
    for (const commandItem of COMMAND_ITEMS) {
      ...
      li.addEventListener('mouseenter', this._handleCommandItemMouseOver.bind(this));
      li.addEventListener('mouseleave', this._handleCommandItemMouseLeave.bind(this));
    ...
    if (this.command) {
      const currentLi = frag.querySelector(`.cb-command-item[data-command="${this.command.getName()}"]`);
  ...
  private async _buildSettingsPanel(): Promise<HTMLElement> {
    ...
    const templates = (await COMMAND_SETTING_TEMPLATES).querySelectorAll<HTMLTemplateElement>(
  ...
  private _closeCommandBar() {
    ...
    overlay.addEventListener('transitionend', function remove(e) {
      if (e.currentTarget === e.target) overlay.removeEventListener('transitionend', remove);
      overlay.remove();
    });
    bar.addEventListener('transitionend', function remove(e) {
      if (e.currentTarget === e.target) bar.removeEventListener('transitionend', remove);
      bar.remove();
    });
  ...
  private _showNewPanel(newPanel: HTMLElement, slideIn: string, slideOut: string) {
  ...
  private _handleFormSubmit(event: Event) {
    ...
    const inputs = this.shadow.querySelectorAll<HTMLInputElement>('#settingsForm [name]');
  ...
  private _handleCommandItemMouseLeave(event: MouseEvent) {
    const info = (event.currentTarget as HTMLElement).querySelector('.cb-command-info') as HTMLElement;
  ...
  private _handleSearch(query: string, panel: HTMLElement) {
    ...
    this.shadow.querySelectorAll<HTMLSpanElement>('.cb-command-name').forEach(el => {
