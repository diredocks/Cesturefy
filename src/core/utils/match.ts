  ...
  switch (algorithm) {
    ...
    default: {
      ...
      for (const g of gestures) {
        ...
        const diffProportion = patternSimilarityByProportion(pattern, g.getPattern());
...
function patternSimilarityByDTW(patternA: Vectors, patternB: Vectors): number {
  ...
  const DTW: number[][] = Array.from(Array(rows), () => Array(columns).fill(Infinity));

  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < columns; j++) {
      const cost = Math.abs(vectorDirectionDifference(
...
function overlapProportion(minA: number, maxA: number, minB: number, maxB: number): number {
