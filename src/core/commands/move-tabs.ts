...
interface MoveTabToSESettings { }

const moveTabToStart: CommandFn<MoveTabToSESettings> = async function (sender) {
...
const moveTabToEnd: CommandFn<MoveTabToSESettings> = async function (sender) {
  if (!sender.tab?.id) return true;
...
interface MoveTabToNewWindowSettings { }

const moveTabToNewWindow: CommandFn<MoveTabToNewWindowSettings> = async function (sender) {
  if (!sender.tab?.id) return true;
...
// --- export commands with default settings ---
