...
interface SaveSettings { promptDialog: boolean; };
...
export const SaveLink = defineCommand(SaveLinkFn, { promptDialog: true }, 'link', ['downloads']);

const SaveScreenshotFn: CommandFn = async function (sender) {
  const screenshotDataUrl = await chrome.tabs.captureVisibleTab({ format: "jpeg" });
...
export const SaveScreenshot = defineCommand(SaveScreenshotFn, {}, 'capture', ['downloads', 'activeTab']);
