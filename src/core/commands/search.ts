...
const SearchClipboardFn: CommandFn<SearchSettings> = async function (sender) {
  ...
  const clipboardText = await sendTabMessage(sender.tab.id, 'clipboardReadText', {});
...
export const SearchClipboard = defineCommand(SearchClipboardFn, {
  searchEngineURL: "",
  openEmptySearch: false,
}, 'clipboard', ['search', 'clipboardRead']);

const SearchTextSelectionFn: CommandFn<SearchSettings> = async function (sender, data) {
  ...
  if (searchEngineURL.includes("%s")) {
    searchEngineURL = searchEngineURL.replace("%s", encodeURIComponent(selectionText));
  }
  else {
...
export const SearchTextSelection = defineCommand(SearchTextSelectionFn, {
  searchEngineURL: "",
  openEmptySearch: false,
}, 'selection', ['search']);
...
const SearchClipboardNewTabFn: CommandFn<SearchNewTabSettings> = async function (sender) {
  if (!sender.tab?.id) return false;
  ...
  const tab = await chrome.tabs.create({ index, active: this.getSetting('focus'), url: "about:blank" });
...
export const SearchClipboardInNewTab = defineCommand(SearchClipboardNewTabFn, {
  searchEngineURL: "",
  openEmptySearch: false,
  position: "default",
  focus: false,
...
const SearchTextSelectionNewTabFn: CommandFn<SearchNewTabSettings> = async function (sender, data) {
  if (!sender.tab?.id || !data) return false;
...
export const SearchTextSelectionInNewTab = defineCommand(SearchTextSelectionNewTabFn, {
  searchEngineURL: "",
  openEmptySearch: false,
  position: "default",
  focus: false,
