...
interface OpenCustomSettings { url?: string; };
...
export const OpenCustomURL = defineCommand(OpenCustomURLFn, { url: "" }, 'open');

const OpenCustomURLNewWindowFn: CommandFn<OpenCustomSettings> = async function () {
  try {
...
export const OpenCustomURLInNewWindow = defineCommand(OpenCustomURLNewWindowFn, { url: "" }, 'open');

const OpenCustomURLNewPrivateWindowFn: CommandFn<OpenCustomSettings> = async function () {
  try {
    await chrome.windows.create({
      url: this.getSetting("url"),
      incognito: true,
...
export const OpenCustomURLInNewPrivateWindow = defineCommand(OpenCustomURLNewPrivateWindowFn, { url: "" }, 'open');
...
const OpenCustomURLNewTabFn: CommandFn<OpenCustomNewTabSettings> = async function (sender) {
  let index;
...
export const OpenCustomURLInNewTab = defineCommand(OpenCustomURLNewTabFn, { position: 'default', focus: false, url: '' }, 'open');
