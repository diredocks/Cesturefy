...
export class RockerController {
  ...
  addEventListener<K extends keyof RockerEvents>(
    event: K,
    cb: RockerEvents[K],
  ) {
  ...
  removeEventListener<K extends keyof RockerEvents>(
    event: K,
    cb: RockerEvents[K],
  ) {
  ...
  private _handleMousedown = (e: MouseEvent) => {
    ...
    if (e.buttons === MouseButton.LEFT + MouseButton.RIGHT) {
      this._events.dispatchEvent(
        e.button === 0 ? "rockerleft" : "rockerright",
        e,
      ); // idk why but left just be 0
  ...
  private _handleContextMenu = (e: MouseEvent) => {
    ...
    if (
      e.button === toSingleButton(MouseButton.RIGHT) &&
  ...
  private _handleClick = (e: MouseEvent) => {
    ...
    if (
      e.button === toSingleButton(MouseButton.LEFT) &&
      e.detail &&
      e.timeStamp === this._lastMouseup &&
      this._preventDefault
    ) {
      e.stopPropagation();
      e.preventDefault();
